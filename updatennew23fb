...
- **Role of this component:**
  - **Listen** to blockchain events (যেমন `BatchCreated`, `BatchTransferred`, `PrescriptionCreated`, ইত্যাদি)।
  - Event গুলোকে **human‑readable UI** তে convert করে:
    - short addresses,
    - timestamps,
...
```text
Created → Verified (QA) → Shipped → In Storage → Dispensed → (Optionally) Recalled
```

Blockchain + UI mix:

- প্রতিটা step usually correspond করে blockchain event এর সাথে:
  - `BatchCreated`
  - `BatchVerified`
  - `BatchShipped`
  - `BatchReceived`
  - `BatchDispensed`
  - `BatchRecalled`
- `BatchTimeline` component এই events read করে:
  - correct **order** + **timestamp** অনুযায়ী একটা **visual timeline graph** বানায়।
...
- **Events:**
  ...
  - `BatchDispensed`
  - `BatchRecalled`
  - Others like `UserRoleUpdated`, `PrescriptionCreated`, etc.
...
Created ──► Verified ──► Shipped ──► Received ──► Dispensed ──► (Recalled?)
   ^           ^            ^            ^             ^
   │           │            │            │             │
   │           │            │            │             │
BatchCreated  BatchVerified  BatchShipped BatchReceived BatchDispensed  (Events)
```

BN+EN:

- উপরকার লাইনে আপনি UI তে যা দেখান (timeline nodes)।
- নিচের events line হল **smart contract events**.
- For any batch:
  - UI timeline = **sorted list of events** with human labels.
